...
class Mesh_A : public Direct3D11Drawable
{
public:
	Mesh_A(Direct3D11Renderer& pd3dRenderer, std::shared_ptr<MeshData>& mesh, std::shared_ptr<Material>& material) : Direct3D11Drawable(pd3dRenderer, mesh, material)
	{
	}
...
class Node
{
	friend class Model_A;

public: 
	Node(std::vector<Mesh_A*> meshPtrs, DirectX::XMMATRIX transform) : meshPtrs(std::move(meshPtrs))
	{
...
public:
	Model_A(Direct3D11Renderer& pd3dRenderer, std::string fileName, std::string folderPath)
	{
	...
	std::shared_ptr<Mesh_A> ParseMesh(Direct3D11Renderer& pd3dRenderer, aiMesh& mesh, const aiMaterial* const* pMaterials, std::string folderPath, int index)
	{
		...
		return std::make_shared<Mesh_A>(pd3dRenderer, meshData, material);
	}
...
private:
	...
	std::vector<std::shared_ptr<Mesh_A>> meshPtrs;
};